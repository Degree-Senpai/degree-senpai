<template>
  ...
  <div>
    ...
    <div @dragover.prevent @drop="schedulerRemove">
      ...
      <!-- SEARCH POPOVER, ALWAYS RENDERED, HIDING CONTENTS IS THE RESPONSIBILITY OF THE COMPONENT -->
      ...
      <div class="columns">
        ...
        <!-- TODO: write what it means for a requirement to be unfulfilled, whether the requirement allows classes
          to be shared with other requirements, and what the user can select -->
    ...
    methods: {
        ...
        filterGroups(groups) {
          return Object.fromEntries(
            Object.entries(groups).filter(([key, value]) => !key.includes('$') && value != null)
          );
        },
        ...
        getRequirementGroup(fullstr) {
          ...
          let majorGroup = this.requirementGroups[names[0]];
          if (majorGroup) {
            return majorGroup[names[1]]
          }
          return null
        },
        ...
        toggleHighlightFulfillment(group, minorGroup) {
          const name = group + "\\" + minorGroup;
          if (name == this.highlightedFulfillment) {
            this.highlightedFulfillment = null;
          } else {
            this.highlightedFulfillment = name;
          }
        },
        ...
        async queryCalculations(fulfill = true, recommend = true) {
            ...
            // console.log("requirements:: " + JSON.stringify(this.requirements) + "\n\ngroups:: " + JSON.stringify(this.requirementGroups))
        },
...
<style scoped>
  ...
  .degree-selection-menu {
    position: absolute;
    background-color: hsla(210, 3%, 48%, 0.5);
    z-index: 999;
  }
  .main-loading h1 {
    font-size:0.9em;
  ...
  .requirements-major-groups {
    background-color: rgba(68, 74, 79, 0.5);
    border-radius: 4px;
    padding: 4px;
    margin: 4px;
  }
  .requirements-major-groups h2 {
    color: #b6bcc1;
    padding-top: 8px;
    font-size: 18px;
    font-weight: 650;
